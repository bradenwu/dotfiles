#!/usr/bin/env bash
# Install ~/.bashrc. Relies on init_shell.sh for helper files it sources.
set -euo pipefail
...
print_info "Installing bash module (mode=$DOTFILES_MODE)"

install_file "configs/bashrc" "$HOME/.bashrc"

ensure_cmd bash "bash is expected to be available on any Unix system"

print_ok "bash module installed"
print_info "Reload with: source ~/.bashrc (or log out/in)"
